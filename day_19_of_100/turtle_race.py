        ...
        for index in range(len(turtles)):
            ...
            if turtles[index].xcor() >= 200:
                if turtle_colors[index]==_user_bet.lower():
                    ...
                    screen.title(f"Congratulations!, You WON the bet, {turtle_colors[index].capitalize()} Turtle won  ")
                else:
                    print(f"Oops!, You LOST the bet, {turtle_colors[index].capitalize()} Turtle won")   
                    screen.title(f"Oops!, You LOST the bet, {turtle_colors[index].capitalize()} Turtle won  ") 
                is_runnung = False
                break
